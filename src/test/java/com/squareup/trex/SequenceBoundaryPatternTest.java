...
/**
 * Unit test the {@link SequenceBoundaryPattern} class.
 *
 * @author <a href="mailto:gabor@squareup.com">Gabor Angeli</a>
 */
class SequenceBoundaryPatternTest {
  /**
  ...
  /**
   * Test the baseline case for the
   * {@link SequenceBoundaryPattern#matches(List, int, Matcher)} method.
   */
  @Test void matchesBasicCaseCaret() {
    SequenceBoundaryPattern pattern = SequenceBoundaryPattern.CARET;
   ...
   * {@link SequenceBoundaryPattern#matches(List, int, Matcher)} method.
   */
  @Test void matchesBasicCaseDollar() {
    SequenceBoundaryPattern pattern = SequenceBoundaryPattern.DOLLAR;
    Matcher<CoreLabelInputToken> matcher = new Matcher<>(pattern, tokens /* "hello world" */);
    assertTrue(pattern.matches(tokens, 2, matcher), "Baseline check");
  }
  ...
  @Test void testToString() {
    assertEquals("^", SequenceBoundaryPattern.CARET.toString());
    assertEquals("$", SequenceBoundaryPattern.DOLLAR.toString());
  }
